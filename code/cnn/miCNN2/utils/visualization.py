"""Module with visualization helper methods."""
import numpy as np
...
def map_to_patch(y, x, f_size=63):
    """Map output coordinates to input coordinates."""
    h_size = (f_size - 1) / 2
    t = lambda v: v * (h_size + 1) - h_size
    return t(y), t(x)


def get_gaussian_lesion_map(lesions, f_size=63, out_size=16, input_size=512):
    size = (input_size, input_size)
    lesion_map = np.zeros(size)
    s = np.zeros(size)
    f = gaussian_filter(shape=(f_size, f_size), sigma=7)
    for y in range(out_size):
        for x in range(out_size):
            ...
            s[yi:yf, xi:xf] += curr_f
            lesion_map[yi:yf, xi:xf] += curr_f * lesions[y, x]
